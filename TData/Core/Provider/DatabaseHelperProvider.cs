...
namespace TData.Core.Provider
{
    internal static partial class DatabaseHelperProvider
    {
        ...
        //TODO: consider skip for insert when return value for oracle given that it need read manually
        internal static Action<object, DbCommand, DbDataReader> LoadOutParameterDelegate(in bool isExecuteNonQuery, in Type type, in DbParameterInfo[] parameters, in DbProvider provider)
        {
            ...
            foreach (var parameter in parameters.Where(x => x.IsOutput))
            {
                ...
                if (provider == DbProvider.Oracle)
                {
                    ...
                    il.Emit(OpCodes.Call, typeof(DatabaseProvider).GetMethod(nameof(GetValueFromOracleParameter), BindingFlags.Static | BindingFlags.NonPublic));
                }
                else
                {
            ...
            else
            {
                ...
                if (type != null)
                {
                    foreach (var item in type.GetProperties(BindingFlags.Public | BindingFlags.Instance))
                    {
                        ...
                        Type effectiveType = Nullable.GetUnderlyingType(item.PropertyType) ?? item.PropertyType;
                        attributes = GetDbParameterAttribute(item);
                    ...
                    if (options.KeyAsReturnValue && options.Provider == DbProvider.Oracle && DbConfig.Tables.TryGetValue(type.FullName!, out var table))
                    {
                        parameters.AddLast(new DbParameterInfo(
                            name: $"{table.Key.Name}_OUTPUT",
                            bindName: null,
                            size: 0,
                            precision: 0,
                            scale: 0,
                            ...
                            dbType: GetEnumValue(DbProvider.Oracle, table.Key.Property.PropertyType),
                            value: null,
                            converter: null));
